...
void memRead(
			...
			uint   group_rem_size_xyz,//it is weight
			uchar  win_size_x,  // weight_w+CONV_GP_SIZE_X-1
			uchar  win_size_y, // it is also weight_h
			uint   win_size_xyz,  
			// Data Ports
			__global lane_data    *restrict bottom, // data source
...

{
	...
	for(unsigned short win_itm_z=0; win_itm_z<weight_dim3/VEC_SIZE; win_itm_z++){
		for(unsigned char  win_itm_y=0; win_itm_y<win_size_y; win_itm_y++){
			//win_size_x= weight_w+(CONV_GP_SIZE_X-1)*stride,because it is a work-grop,so in one time,
			//extract the suitable for all groups
	...
	Group:for(unsigned int out_idx_xyz=0; out_idx_xyz<(weight_dim4_div_lane*group_num_y*group_num_x); out_idx_xyz++){
		...
		if(split==0)
			...
			data_offset = weight_dim3/VEC_SIZE;	// the upper half of the output feature maps depend on the upper half of the input
				...
				if(gp_num_x==group_num_x-1)//win_size_x :one group's size;group_rem_size_x:数据不重复的size
				...
				Item:for(unsigned int  win_itm_xyz=0; win_itm_xyz<item_loop_bound; win_itm_xyz++){
							...
							if(win_itm_z<weight_dim3/VEC_SIZE){// 死循环
							...
							if(gp_item_idx_x==0){
								//output_idx_dim3是跳Vec的，共weight_n/Vec_size个，
							...
							if(gp_num_x*CONV_GP_SIZE_X+gp_item_idx_x<conv_x){
								...
								if(output_idx_dim1==0 && output_idx_dim2==0 && output_idx_dim3==0){
									bias_ch_in = bias[out_idx_z];//一次 一组lane_num的数据
								...
								//因为每个LANE里的图像数据是一样的，所以直接把data_vec拷贝多份，写到channel里
								#pragma unroll
								...
								// weight and bias fetcher,  weight_ch_vec 仅仅是一次 work item运算的数据，即1*1*VEC_SIZE
								...
								// used as output loop counters   output_idx_dim3++，意味着 完成了一个weigh_w*h*n/VEC_size的参数读取
		...
		// used as virtual group loop counters for winbuf loading operations更新 下一个group的data数据
			...
			)
{
	...
	for(unsigned int k=0; k<output_num; k++){
		...
		for(unsigned char ll=0; ll<LANE_NUM; ll++){
			...
			#pragma unroll
			for(unsigned int p=0; p<PIPE_DEPTH; p++){//干啥用的？
		...
		for(int j=0; j<conv_loop_cnt; j++){
			...
			#pragma unroll
			for(unsigned char ll=0; ll<LANE_NUM; ll++){//实现了 并行CU
		...
		for(unsigned char ll=0; ll<LANE_NUM; ll++){
			...
			for(unsigned i=0; i<PIPE_DEPTH; i++){
				conv_out[ll] += accum_piped[ll][i];//将流水线中的数据全部加起来，就是卷积后的输出，最后加bias
			}
			
			if(conv_out[ll]>=0)
				conv_sign_exten[ll] = 0x00;
			else
				conv_sign_exten[ll] = ~(0xFFFFFFFF>>(frac_w+frac_din-frac_dout-1));//{7,  0, -2}
...
void maxPool(
			// Params Ports
			uint  input_num,    //conv_x*y*weight_m/LANE_NUM
			uchar line_size,  //conv_x line_size should be no larger than POOL_LBUF_DEPTH
			...
			)
{
	...
	DPTYPE line_buf_1[LANE_NUM][POOL_LBUF_DEPTH];//直接这样定义 有点欠妥当，应该先初始化一下，默认是0吗？
	uchar  line_buf_ptr;
	uchar  col_pool_cnt;
	uchar  row_pool_cnt;
	uchar  row_cnt;
