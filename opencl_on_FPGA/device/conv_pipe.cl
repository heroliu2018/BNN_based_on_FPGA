...

{
    ...
    unsigned int seed_weight = 65535;//n=16 bit, 2^n-1 states,set the middle
    unsigned int seed_bias = 65535;

    //short    random_weight = 0;
    short    gauss_weight = 0;
    ...
    gauss_weight = gauss_random_generator(load, seed_weight);
    printf("gauss_original1 %d\n",gauss_weight);
    load = 0;
    gauss_weight = gauss_random_generator(load, seed_weight);
    printf("gauss_original2 %d\n",gauss_weight);
    ...
    for(unsigned int sample_i =0; sample_i <sample_num; sample_i++){
        ...
        Group:for(unsigned int out_idx_xyz=0; out_idx_xyz<(weight_dim4_div_lane*group_num_y*group_num_x); out_idx_xyz++){
	        ...
	        Item:for(unsigned int  win_itm_xyz=0; win_itm_xyz<item_loop_bound; win_itm_xyz++){       
		        ...
		        if(gp_item_idx_x==0){
                    ...
                    for(unsigned char vec_i =0; vec_i <LANE_NUM; vec_i++){
                            for(unsigned char vec_j =0; vec_j <VEC_SIZE; vec_j++){
                                    if(sample_debug){
                                        temp_weight.lane[vec_i].data[vec_j]=32*weight_mean_ch_vec.lane[vec_i].data[vec_j];
                                        #ifdef DEBUG_MEMRD
                                    ...
                                    {       
                                        ...
                                        //all multify 32  ci =bi/32   ~N(0,1)                                    
                                        temp_weight.lane[vec_i].data[vec_j] = 32*weight_mean_ch_vec.lane[vec_i].data[vec_j] + gauss_weight*weight_var_ch_vec.lane[vec_i].data[vec_j];
                                        #ifdef DEBUG_RANDOM
                                        if((out_idx_xyz==0)&&(win_itm_xyz==0))
                                            printf("weight_gauss= %d  ", gauss_weight);
                                        #endif
                                        //gauss_weight =0;
		        ...
		        if(gp_num_x*CONV_GP_SIZE_X+gp_item_idx_x<conv_x){
			        ...
			        if(output_idx_dim1==0 && output_idx_dim2==0 && output_idx_dim3==0){
                        ...
                        //if(seed_bias < 2)
                        //        seed_bias = 1024*64-1;                                                                  
                        gauss_bias = gauss_random_generator(load, seed_bias);
                        //printf("bias_random_seed: %d", random_bias);
                        load =0;
                        #pragma unroll      
                        for(unsigned char vec_i =0; vec_i <LANE_NUM; vec_i++){
                            if(sample_debug){
                                temp_bias.lane[vec_i] = 32*bias_mean_ch_in.lane[vec_i];
                                #ifdef DEBUG_MEMRD        
                                //if((out_idx_xyz==0)&&(win_itm_xyz==0))
                                    printf("bias_mean= %f ll =%d  ", temp_bias.lane[vec_i]/32.0,vec_i);
                                #endif
                            }    
                            else{   
                                gauss_bias = gauss_random_generator(load, seed_bias);                                                                 
                                
                                temp_bias.lane[vec_i] = 32*bias_mean_ch_in.lane[vec_i] + gauss_bias*bias_var_ch_in.lane[vec_i];//all multify 16
                                #ifdef DEBUG_RANDOM
                                if((out_idx_xyz==3)&&(win_itm_xyz==0))
                                    printf("bias_gauss= %d  ", gauss_bias);
                                #endif
                            }
                        }
	        ...
	        // usesd as virtual group loop counters
        ...
        for(unsigned int k=0; k<output_num; k++){	
	        ...
	        for(int j=0; j<conv_loop_cnt; j++){
		        ...
		        for(unsigned char ll=0; ll<LANE_NUM; ll++){
			        ...
			        #ifdef DEBUG_CONV
			        //if((k==0)&&(j<conv_loop_cnt/2)){
			        //	printf("Conv orignal: data=%f weight=%f (loop=%d, lane= %d)\n", (float)mac_data.lane[ll].data[0]/64.0, (float)mac_weight.lane[ll].data[0]/32.0, j, ll);
			        //}
			        #endif
		        }
	        }// end of conv loop
	        ...
	        for(unsigned char ll=0; ll<LANE_NUM; ll++){
                ...
                conv_sum_bias[ll] =(bias[ll]>>(frac_w-frac_dout-1));//bias need to handle 
		        ...
		        #ifdef DEBUG_CONV
		        if(k < 20){
			        printf("Conv result: k =%d  conv_mul=%f bias_org=%d bias=%f sum=%f final=%f ll= %d\n",k,(float)conv_out[ll],bias[ll],(float)conv_sum_bias[ll],(float)conv_with_rnd_bit[ll],(float)conv_final[ll],ll);//attention!!!!   ll must be put in the end
			     }
		        #endif
	        }
				...
				)
{
	...
	if((global_z-padd_offset)<(out_dim3*sample_num) && (global_z>=padd_offset)){              
		...
		//if((global_z-padd_offset) == 0){
			//for(unsigned char ll=0; ll<LANE_NUM; ll++){
		printf("MemWr results=%f (x=%d, y=%d, z=%d, ll=%d)----",(float)buffer[local_z], global_x, global_y, global_z, 0);
			//}
		//	}
		#endif
	}	
