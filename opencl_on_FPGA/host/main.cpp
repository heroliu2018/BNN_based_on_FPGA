//////////////////////////////////////////
//
// OpenCL host program for Mnist inference
// based on intel FPGA
//
// author: Yingh Liu
// data :2019.4.2
...
#define IN_BUF_SIZE    28*28*64*SAMPLE_NUM  // Note: the buffer size should be large enough to hold all temperary results
#define OUT_BUF_SIZE   28*28*64*SAMPLE_NUM
#define FC_BUF_SIZE    400*MAX_BATCH_SIZE*SAMPLE_NUM
...
const char *mean_data_file_path    = "/home/zzw/01.FPGA_BNN/05.CNN_based_on_opencl/02.Paper_materials/02.CNN_Based_on_OpenCL/PipeCNN-master/data/imagenet/mean_data.dat";
const char *synset_word_file_path  = "/home/zzw/01.FPGA_BNN/05.CNN_based_on_opencl/02.Paper_materials/02.CNN_Based_on_OpenCL/PipeCNN-master/data/imagenet/synset_words.txt";
const char *LabelPath		       = "/home/zzw/01.FPGA_BNN/05.CNN_based_on_opencl/02.Paper_materials/02.CNN_Based_on_OpenCL/PipeCNN-master/data/imagenet/val.txt";
...
#define LAYER_NUM         8
#define CONV_NUM          5
const char *weight_file_path = "/home/zzw/01.FPGA_BNN/05.CNN_based_on_opencl/02.Paper_materials/02.CNN_Based_on_OpenCL/PipeCNN-master/data/data_alex/weights.dat";
const char *input_file_path = "/home/zzw/01.FPGA_BNN/05.CNN_based_on_opencl/02.Paper_materials/02.CNN_Based_on_OpenCL/PipeCNN-master/data/data_alex/image.dat";
const char *ref_file_path = "/home/zzw/01.FPGA_BNN/05.CNN_based_on_opencl/02.Paper_materials/02.CNN_Based_on_OpenCL/PipeCNN-master/data/data_alex/fc8.dat";
const char *dump_file_path = "./result_dump.txt";
*/


//2-layers full connected network
// File size is in num of DTYPE numbers
#define IMAGE_FILE_SIZE   (28*28*1)
#define WEIGHTS_FILE_SIZE (794*400*2+410*2)  //bytes
#define LAYER_NUM         2
#define CONV_NUM          0

const char *weight_file_path = "/home/zzw/01.FPGA_BNN/06.Realizing/02.full_connected_opencl/data/data_fullconn/weights.dat";
const char *input_file_path = "/home/zzw/01.FPGA_BNN/06.Realizing/02.full_connected_opencl/data/data_fullconn/image.dat";
const char *ref_file_path = "/home/zzw/01.FPGA_BNN/06.Realizing/02.full_connected_opencl/data/data_fullconn/fc8.dat";
const char *dump_file_path = "./result_dump.txt";
...
scoped_array<cl_mem> weights_buf_mean;
scoped_array<cl_mem> weights_buf_var;
scoped_array<cl_mem> bias_buf_mean;
scoped_array<cl_mem> bias_buf_var;
scoped_array<cl_mem> fc_1_buf;//for ping-pong buffer
scoped_array<cl_mem> fc_2_buf;//for ping-pong buffer
...
DTYPE *image;
DTYPE *data_init;
DTYPE *weight_conv_mean[MAX_LAYER_NUM];
DTYPE *weight_conv_var[MAX_LAYER_NUM];
DTYPE *bias_conv_mean[MAX_LAYER_NUM];
DTYPE *bias_conv_var[MAX_LAYER_NUM];
DTYPE *output;
...
void dumpResult();
void reorderWeights(DTYPE *weights, DTYPE *weight_buf_mean, DTYPE *weight_buf_var, unsigned dim1, unsigned dim2, unsigned dim3, unsigned dim4, unsigned dim3_original, unsigned dim4_original, unsigned offset, unsigned padding_offset, unsigned vecSize, unsigned laneNum);
void reorderBias(DTYPE *dataIn, DTYPE *bias_mean,DTYPE *bias_var, unsigned offset, unsigned padding_offset, unsigned dim4, unsigned dim4_original, unsigned laneNum);
...
int main(int argc, char** argv)
{
	...
	unsigned char batch_size_in_dim;//when pooling , to control batch size
	unsigned char batch_indx_dim1;//when pooling , to control batch size
	unsigned char batch_indx_dim2;//when pooling , to control batch size
	...
	printf("***************************************************\n");
	printf("BNN Inference on OpenCL-Based FPGA Accelerator \n");
	printf("***************************************************\n");
	...
	weights_buf_mean.reset(num_devices*LAYER_NUM);
        weights_buf_var.reset(num_devices*LAYER_NUM);
	bias_buf_mean.reset(num_devices*LAYER_NUM);
        bias_buf_var.reset(num_devices*LAYER_NUM);
 ...
 // IntelFPGA
			...
			weights_buf_mean[i*LAYER_NUM+j] = clCreateBuffer(context, CL_MEM_READ_ONLY, weight_buf_size* sizeof(DTYPE), NULL, &status);
			checkError(status, "Failed to create buffer for weights in layer");
                        weights_buf_var[i*LAYER_NUM+j] = clCreateBuffer(context, CL_MEM_READ_ONLY, weight_buf_size* sizeof(DTYPE), NULL, &status);
			checkError(status, "Failed to create buffer for weights in layer");

			// Bias buffers for each layer
			bias_buf_mean[i*LAYER_NUM+j] = clCreateBuffer(context, CL_MEM_READ_ONLY, layer_config[j][bias_size] * sizeof(DTYPE), NULL, &status);
			checkError(status, "Failed to create buffer for bias in layer");
                        bias_buf_var[i*LAYER_NUM+j] = clCreateBuffer(context, CL_MEM_READ_ONLY, layer_config[j][bias_size] * sizeof(DTYPE), NULL, &status);
			checkError(status, "Failed to create buffer for bias in layer");



			// Initializing all weights buffers, blocking write is used
			status = clEnqueueWriteBuffer(que_memRd[i], weights_buf_mean[i*LAYER_NUM+j], CL_TRUE, 0, weight_buf_size*sizeof(DTYPE), weight_conv_mean[j], 0, NULL, NULL);
			checkError(status, "Failed to transfer weight");
            status = clEnqueueWriteBuffer(que_memRd[i], weights_buf_var[i*LAYER_NUM+j], CL_TRUE, 0, weight_buf_size*sizeof(DTYPE), weight_conv_var[j], 0, NULL, NULL);
			checkError(status, "Failed to transfer weight");

			status = clEnqueueWriteBuffer(que_memRd[i], bias_buf_mean[i*LAYER_NUM+j], CL_TRUE, 0, layer_config[j][bias_size] * sizeof(DTYPE), bias_conv_mean[j], 0, NULL, NULL);
			checkError(status, "Failed to transfer bias");
                        status = clEnqueueWriteBuffer(que_memRd[i], bias_buf_var[i*LAYER_NUM+j], CL_TRUE, 0, layer_config[j][bias_size] * sizeof(DTYPE), bias_conv_var[j], 0, NULL, NULL);
			checkError(status, "Failed to transfer bias");
 ...
 // IntelFPGA
	...
	unsigned char  conv_win_size_dim1, conv_win_size_dim2;//保存卷积核正常滑动的group所需的data size, the size of each group move
	unsigned int   conv_win_size_dim1x2x3;
	unsigned char  conv_group_rem_dim1, conv_group_rem_dim2;//因为卷积步长的问题，这个保存 卷积核滑动时  无重合的情况
	...
	for(unsigned i = 0; i < num_devices; ++i) {
        ...
        unsigned int sample_n = SAMPLE_NUM;
        unsigned char sample_control = 0;
		...
		for(unsigned char j = 0; j < LAYER_NUM; ++j){
			...
			for(unsigned k = 0; k < iter_num; ++k){
                ...
                status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_uint), &sample_n);
			    checkError(status, "Failed to set argument %d of kernel memRd", argi - 1);
                            
                status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_uchar), &sample_control);
			    checkError(status, "Failed to set argument %d of kernel memRd", argi - 1);
			    
			    unsigned int delay =10;
			    status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_uint), &delay);
			    checkError(status, "Failed to set argument %d of kernel memRd", argi - 1);
			    ...
			    status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_mem), &weights_buf_mean[i*LAYER_NUM+j]);
			    checkError(status, "Failed to set argument %d kernel memRd", argi - 1);
                            status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_mem), &weights_buf_var[i*LAYER_NUM+j]);
			    checkError(status, "Failed to set argument %d kernel memRd", argi - 1);

			    status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_mem), &bias_buf_mean[i*LAYER_NUM+j]);
			    checkError(status, "Failed to set argument %d kernel memRd", argi - 1);
                            status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_mem), &bias_buf_var[i*LAYER_NUM+j]);
			    checkError(status, "Failed to set argument %d kernel memRd", argi - 1);
                ...
                status = clSetKernelArg(knl_conv[i], argi++, sizeof(cl_uint), &sample_n);
			    checkError(status, "Failed to set argument %d of kernel conv", argi - 1);
                ...
                status = clSetKernelArg(knl_memWr[i], argi++, sizeof(cl_uint), &sample_n);
			    checkError(status, "Failed to set argument %d of kernel memWr", argi - 1);
			    ...
			    knl_memWr_global_size[2] = layer_config[j][weight_m]*sample_n; // pool_z equals original weight_m, new weight_m is divisible by LANE_NUM
    ...
    // IntelFPGA
                ...
                printf("already start the memWR\n");
                ...
                */
			    ...
			    //}
                printf("end of batch iteration\n");
                printf("knl_memWr_global_size[2]  %ld\n",knl_memWr_global_size[2]);
...
void readDataBack()
{
	...
	if(LAYER_NUM<CONV_NUM){ // verify conv results
		read_buf_size = output_config[output_w]*output_config[output_h]*output_config[output_n]*SAMPLE_NUM;
	}
	else // verify the last conv and all fc results
		read_buf_size = output_config[output_w]*output_config[output_h]*output_config[output_n]*input_config[batch_size]*SAMPLE_NUM;
	...
	if(layer_config[LAYER_NUM-1][memwr_dst] == 2){
		...
		status = clEnqueueReadBuffer(que_memWr[0], fc_2_buf[0], CL_FALSE,          // read from device0
			0, sizeof(DTYPE) * read_buf_size, (void *)output, 0, NULL, &finish_event[0]);
		checkError(status, "Failed to set transfer output data");
		for(unsigned int p_i=0;p_i<10;p_i++){
		    printf("%d ", DTYPE(output[p_i]));
		}
	}
...
void verifyResult(int num)
{
    ...
    softmax(output_one_item, output_reorder);   
    printf("\n*************Predicted Result**************\n");
    float result_mean[batch_item_size];
	unsigned int arg_max = 0;
    for(unsigned sample_i=0; sample_i < SAMPLE_NUM; sample_i++){
        float arg_temp = output_reorder[sample_i*batch_item_size + 0];       
	    for (unsigned int j = 1; j < batch_item_size; j++){
                    //printf("result_temp=%d  ",output_reorder[sample_i*batch_item_size + j]);
                    if(output_reorder[sample_i*batch_item_size + j] > arg_temp){
                            arg_temp = output_reorder[sample_i*batch_item_size + j];
                            arg_max = j;
                    }
        }
        //printf("\n******************************************* ");
        //printf("\nthe %dth sample result:the number is  %d \n",sample_i+1,arg_max);
        printf("%d  ",arg_max);
        //printf("\n******************************************* \n");
        arg_max =0;        		
	}
	for(unsigned i =0;i<batch_item_size;i++)
	    result_mean[i]=0;
	for(unsigned sample_i=0; sample_i < SAMPLE_NUM; sample_i++){
        for (unsigned int j = 0; j < batch_item_size; j++){
            result_mean[j] =result_mean[j] + output_reorder[j+sample_i*batch_item_size];
        }
    }
    float mean_temp=result_mean[0];
    for (unsigned int j = 1; j < batch_item_size; j++){
        if(result_mean[j] > mean_temp){
            mean_temp = result_mean[j];
            arg_max = j;
        }
    }
    printf("\nthe mean result is %d \n",arg_max);
...
int prepare()
{
	...
	for(unsigned ll=0; ll<LAYER_NUM; ll++){
		...
		if(layer_config_original[ll][pool_on] && ((layer_config_original[ll][pool_x]!=(layer_config_original[ll][conv_x]-layer_config_original[ll][pool_size])/layer_config_original[ll][pool_stride]+1)
			...
			return 1;
                ...
                //Control the stride of the work group
	...
	if(LAYER_NUM>=CONV_NUM)// For last conv and all fc layers, all batch results are read back
		output_size = output_config[output_w]*output_config[output_h]*output_config[output_n]*input_config[batch_size]*SAMPLE_NUM;
	else // For other conv layers, only one item of
		output_size = output_config[output_w]*output_config[output_h]*output_config[output_n]*SAMPLE_NUM;

	godref_size = output_config[output_w]*output_config[output_h]*output_config[output_n]*SAMPLE_NUM;
	...
	if(weights == NULL || image == NULL || data_init == NULL || output == NULL || output_one_item == NULL || output_reorder == NULL)// golden_ref == NULL ||
	{
	...
	for(int j=0; j<LAYER_NUM; j++){
		...
		weight_conv_mean[j] = (DTYPE *)alignedMalloc(sizeof(DTYPE)*weight_size, DMA_ALIGNMENT);
                weight_conv_var[j] = (DTYPE *)alignedMalloc(sizeof(DTYPE)*weight_size, DMA_ALIGNMENT);
		bias_conv_mean[j]   = (DTYPE *)alignedMalloc(sizeof(DTYPE)*layer_config[j][bias_size], DMA_ALIGNMENT);
                bias_conv_var[j]   = (DTYPE *)alignedMalloc(sizeof(DTYPE)*layer_config[j][bias_size], DMA_ALIGNMENT);

		memset(weight_conv_mean[j], 0, sizeof(DTYPE)*weight_size);             // reset all value (include padding value) to zero
                memset(weight_conv_var[j], 0, sizeof(DTYPE)*weight_size);
		memset(bias_conv_mean[j], 0, sizeof(DTYPE)*layer_config[j][bias_size]);// reset all value (include padding value) to zero
                memset(bias_conv_var[j], 0, sizeof(DTYPE)*layer_config[j][bias_size]);

		if(weight_conv_mean[j] == NULL || weight_conv_var[j] == NULL || bias_conv_mean[j] == NULL || bias_conv_var[j] == NULL )
		{
			printf("Not enough memory !!!");
			for(int i=0; i<=j; i++){
				alignedFree(weight_conv_mean[i]);
                                alignedFree(weight_conv_var[i]);
				alignedFree(bias_conv_mean[i]);
                                alignedFree(bias_conv_var[i]);
			}
			return 1;
		}
	}
	...
	if(WEIGHTS_FILE_SIZE!=(file_size/(sizeof(DTYPE))))
		printf("Warning: weight file size does not match user configuration !!!%d----%ld\n",WEIGHTS_FILE_SIZE,file_size/(sizeof(DTYPE)));
    	bin_file_r.close();
    }
    else
...
*/
	// Layer-1
	reorderWeights(weights, weight_conv_mean[0],weight_conv_var[0], layer_config[0][weight_w], layer_config[0][weight_h], layer_config[0][weight_n], layer_config[0][weight_m], layer_config_original[0][weight_n], layer_config_original[0][weight_m], ptr, padding_offset[0], VEC_SIZE, LANE_NUM);
	ptr+=2*layer_config[0][weight_w]*layer_config[0][weight_h]*layer_config_original[0][weight_n]*layer_config_original[0][weight_m];
	reorderBias(weights, bias_conv_mean[0],bias_conv_var[0],ptr, padding_offset[0], layer_config[0][bias_size], layer_config_original[0][bias_size], LANE_NUM);
	ptr+=2*layer_config_original[0][bias_size];
	...
	for(unsigned j=1; j<LAYER_NUM; j++){
		...
		reorderWeights(weights, weight_conv_mean[j],weight_conv_var[j], layer_config[j][weight_w], layer_config[j][weight_h], layer_config[j][weight_n], layer_config[j][weight_m], layer_config_original[j][weight_n], layer_config_original[j][weight_m], ptr, padding_offset[j], VEC_SIZE, LANE_NUM);
		ptr+=2*layer_config[j][weight_w]*layer_config[j][weight_h]*layer_config_original[j][weight_n]*layer_config_original[j][weight_m];
		reorderBias(weights, bias_conv_mean[j],bias_conv_var[j], ptr, padding_offset[j], layer_config[j][bias_size], layer_config_original[j][bias_size], LANE_NUM);
		ptr+=2*layer_config_original[j][bias_size];
	}
...
void reorderWeights(DTYPE *weights, DTYPE *weight_buf_mean, DTYPE *weight_buf_var,unsigned dim1, unsigned dim2, unsigned dim3, unsigned dim4, unsigned dim3_original, unsigned dim4_original, unsigned offset, unsigned padding_offset, unsigned vecSize, unsigned laneNum){
				...
				for(unsigned j = 0; j<dim1; j++){
					for(unsigned ll = 0; ll<laneNum; ll++){
						for(unsigned k = 0; k<vecSize; k++){
							weight_buf_mean[m*dim1*dim2*dim3*laneNum + n*dim1*dim2*vecSize*laneNum + i*dim1*vecSize*laneNum + j*vecSize*laneNum + ll*vecSize + k]
    ...
    offset += dim1*dim2*dim3_original*dim4_original;
    memset(copy_with_padding, 0, sizeof(DTYPE)*dim1*dim2*dim3*dim4);
    //save variance of weight
    for(unsigned m = 0; m<dim4_original; m++){
		for(unsigned n = 0; n<dim3_original; n++){
			for(unsigned i = 0; i<dim2; i++){
				for(unsigned j = 0; j<dim1; j++){
		...
		for(unsigned n = 0; n<(dim3/vecSize); n++){
			for(unsigned i = 0; i<dim2; i++){
				for(unsigned j = 0; j<dim1; j++){
					for(unsigned ll = 0; ll<laneNum; ll++){
						for(unsigned k = 0; k<vecSize; k++){
							weight_buf_var[m*dim1*dim2*dim3*laneNum + n*dim1*dim2*vecSize*laneNum + i*dim1*vecSize*laneNum + j*vecSize*laneNum + ll*vecSize + k]
													= (DTYPE) copy_with_padding[(m*laneNum+ll)*dim3*dim2*dim1 + (n*vecSize+k)*dim1*dim2 + i*dim1 + j];
						}
					}
				}
...
void reorderBias(DTYPE *dataIn, DTYPE *bias_mean, DTYPE *bias_var,unsigned offset, unsigned padding_offset, unsigned dim4, unsigned dim4_original, unsigned laneNum){
	...
	for(unsigned m = 0; m<(dim4/laneNum); m++){
		for(unsigned ll = 0; ll<laneNum; ll++){
			bias_mean[m*laneNum + ll] = (DTYPE) copy_with_padding[m*laneNum + ll];
		}
	}
    memset(copy_with_padding, 0, sizeof(DTYPE)*dim4);
    offset += dim4_original;
    memcpy(copy_with_padding+padding_offset, dataIn+offset, sizeof(DTYPE)*dim4_original);
	// save variance of bias
	for(unsigned m = 0; m<(dim4/laneNum); m++){
		for(unsigned ll = 0; ll<laneNum; ll++){
			bias_var[m*laneNum + ll] = (DTYPE) copy_with_padding[m*laneNum + ll];
		}
	}
	// release resource
	free(copy_with_padding);
...
void extractOutput(DTYPE *output, DTYPE *output_one_item, unsigned item_num, unsigned batch_size, unsigned dim1, unsigned dim2, unsigned dim3){
    ...
    for(unsigned sample_i=0; sample_i <SAMPLE_NUM; sample_i++){
	        for(unsigned k = 0; k<dim3; k++){
	            output_one_item[sample_i*dim1*dim2*dim3 + k] = output[sample_i*dim1*dim2*dim3 + k];
	            printf("%d ",output_one_item[sample_i*dim1*dim2*dim3 + k]);
            }
    }
    /*
    for(unsigned sample_i=0; sample_i <SAMPLE_NUM; sample_i++){
	        for(unsigned k = 0; k<(dim3/VEC_SIZE); k++){//using VEC_SIZE, because the output is possible to be used in the next layer ,so it should be reshaped to VEC
		        for(unsigned i = 0; i<dim2; i++){
			        for(unsigned j = 0; j<dim1; j++){
				        for(unsigned vv = 0; vv<VEC_SIZE; vv++){
					        output_one_item[sample_i*dim1*dim2*dim3 + k*dim2*dim1*VEC_SIZE + i*dim1*VEC_SIZE + j*VEC_SIZE + vv]
						        = output[sample_i*dim1*dim2*dim3 + k*dim2*dim1*batch_size_in_dim*batch_size_in_dim*VEC_SIZE + (i+batch_indx_dim2*dim2)*batch_size_in_dim*dim1*VEC_SIZE + (j+batch_indx_dim1*dim1)*VEC_SIZE + vv];
						    printf("%d ",output_one_item[sample_i*dim1*dim2*dim3 + k*dim2*dim1*VEC_SIZE + i*dim1*VEC_SIZE + j*VEC_SIZE + vv]);
				        }
			        }
		        }
	        }
        ...
        for(unsigned i = 0; i<dim2; i++){
	        for(unsigned j = 0; j<dim1; j++){
		        for(unsigned k = 0; k<(dim3/VEC_SIZE); k++){
			        for(unsigned vv = 0; vv<VEC_SIZE; vv++){
				        output_reorder[sample_i*dim1*dim2*dim3 + (k*VEC_SIZE+vv)*dim2*dim1 + i*dim1 + j]
					        = output[sample_i*dim1*dim2*dim3 + k*dim2*dim1*VEC_SIZE + i*dim1*VEC_SIZE + j*VEC_SIZE + vv];
					    printf("%d ",output_reorder[sample_i*dim1*dim2*dim3 + (k*VEC_SIZE+vv)*dim2*dim1 + i*dim1 + j]);
			        }
		        }
	        }
        }
...
void softmax(DTYPE *output_reorder , DTYPE *output)
{
    unsigned int result_size = output_config[output_n];
    printf("\nsoftmax:");
    for(unsigned sample_i=0; sample_i <SAMPLE_NUM; sample_i++)//multiple sampling
    {   
        unsigned int i;
	    float data_max=0.0;
	    float data_exp=0.0;
	    ...
	    {
	        if(data_max<output_reorder[sample_i*result_size + i])
	            data_max=output_reorder[sample_i*result_size + i];
	           
	    }
        for(i=0;i<result_size;i++)
	    {
	        data_exp=exp((float)output_reorder[sample_i*result_size + i]-data_max);
	        sum_exp += data_exp;
            ...
            for(i=0;i<result_size;i++)
	    {
	        ...
	        output[sample_i*result_size + i]=data_exp / sum_exp*100.0;
	        printf("softmax %d  ",output[sample_i*result_size + i]);
        }
   }
}
...
void cleanup()
{
	...
	for(unsigned i = 0; i < num_devices; ++i) {
		...
		if(weights_buf_mean && weights_buf_mean[i]) {
			clReleaseMemObject(weights_buf_mean[i]);
		}
                if(weights_buf_var && weights_buf_var[i]) {
			clReleaseMemObject(weights_buf_var[i]);
		}
		if(bias_buf_mean && bias_buf_mean[i]) {
			clReleaseMemObject(bias_buf_mean[i]);
		}
                if(bias_buf_var && bias_buf_var[i]) {
			clReleaseMemObject(bias_buf_var[i]);
		}
	...
	alignedFree(data_init);
	for(int j=0; j<LAYER_NUM; j++){
		alignedFree(weight_conv_mean[j]);
                alignedFree(weight_conv_var[j]);
		alignedFree(bias_conv_mean[j]);
                alignedFree(bias_conv_var[j]);
	}
	//alignedFree(golden_ref);
	alignedFree(output);
	alignedFree(output_reorder);
